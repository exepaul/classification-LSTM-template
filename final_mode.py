    ...
    with tf.Session() as sess:
        ...
        for i in range(epoch):
            for j in range(iteration):
                ...
                tweets_datao = padding_data([aa for aa, bb in batch])['input']
                labels_datao = [labels_datr.index(bb) for aa, bb in batch]

                out_a=sess.run(model.out, feed_dict={model.placeholder['input']: tweets_datao,
                                                         model.placeholder['output']: labels_datao})
...
if __name__ == '__main__':
    model = LSTMclassifier(10, len(labels_datr))
    out = rand_exe(model)
